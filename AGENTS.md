...
9. 有没有可能影响其他模块。
```
“本次改动如何人工验证检查”必须写成可执行步骤，不能只写“运行测试”或“检查功能是否正常”。
...
4. 需要在 Inspector 中确认哪些组件或引用。
5. 具体点击或操作步骤。
6. 正常情况下应该看到什么现象。
7. Console 中应该出现什么日志，或者不应该出现什么报错。
8. 如果验证失败，优先检查哪些文件、Prefab、组件或配置。
...
所有 AI 输出都必须方便项目负责人复核。
代码改动不是最终交付，能被人工验证、能被集成、能被回滚，才算完成本次任务。
