...
namespace TreasureArenaMR.MapEditor.Editor
{
    public static class MapEditorRuntimeSceneSetup
    {
        ...
        private static void AddBrushFromPrefab(SerializedProperty brushes, string prefabPath)
        {
            ...
            brush.FindPropertyRelative("has_collider").boolValue = marker != null ? marker.has_collider : hasCollider;
            brush.FindPropertyRelative("object_type").enumValueIndex = marker != null ? (int)marker.object_type : (int)MapObjectType.StaticObstacle;
            brush.FindPropertyRelative("interaction_type").enumValueIndex = marker != null ? (int)marker.interaction_type : (int)MapInteractionType.None;
            brush.FindPropertyRelative("default_scale").vector3Value = Vector3.one;
            brush.FindPropertyRelative("default_rotation").vector3Value = Vector3.zero;
            brush.FindPropertyRelative("is_movable").boolValue = marker != null && marker.is_movable;
            brush.FindPropertyRelative("is_grabbable").boolValue = marker != null && marker.is_grabbable;
            brush.FindPropertyRelative("is_openable").boolValue = marker != null && marker.is_openable;
            brush.FindPropertyRelative("is_shootable").boolValue = marker == null || marker.is_shootable;
            brush.FindPropertyRelative("blocks_bullet").boolValue = marker == null || marker.blocks_bullet;
            brush.FindPropertyRelative("decal_enabled").boolValue = marker == null || marker.decal_enabled;
            brush.FindPropertyRelative("mass").floatValue = marker != null ? marker.mass : 0f;
        }
