...
namespace TreasureArenaMR.Network
{
    ...
    public sealed class SandboxNetworkListener : NetworkEventsListener
    {
        ...
        private void SpawnPlayer(NetworkSandbox sandbox, string playerId, NetickPlayer netPlayer)
        {
            ...
            Camera playerCamera = Camera.main;  // Pico 头显
            Vector3 headPos = playerCamera != null ? playerCamera.transform.position : spawnPos;
            var playerObj = sandbox.NetworkInstantiate(_playerPrefab, headPos, Quaternion.identity, netPlayer);
